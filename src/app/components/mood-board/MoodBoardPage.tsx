...
import { uploadMoodBoardImage, uploadImageFromUrl } from "@/app/utils/mood-board-storage"
...
export function MoodBoardPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (isLoggedIn && user) {
        ...
        const { url: storageUrl } = await uploadMoodBoardImage(file, user.email)
        if (storageUrl) {
          await addImagePin(storageUrl)
          continue
        }
      }
  ...
  const handleUrlAdd = async () => {
    ...
    // If logged in, upload URL image to Supabase Storage for persistence
    if (isLoggedIn && user) {
      const { url: storageUrl } = await uploadImageFromUrl(imageUrl, user.email)
      await addImagePin(storageUrl || imageUrl)
    } else {
          ...
          >
            ...
            >
              ...
              <p className="font-['DM_Sans',sans-serif] text-[13px] text-[#6b6860] leading-[1.6] mb-6 max-w-[300px] mx-auto">
                This pin will be permanently removed from your board.
              </p>
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => deletePin(deletePinId)}
