...
export function StepWizard({ totalSteps, currentStep, onNext, onBack, canProceed, children, showResults }: StepWizardProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f0ede6]">
      {/* Header */}
      <header className="flex items-center justify-between px-6 md:px-10 py-4 border-b border-[#d8d3c8]">
        <Link to="/" className="font-['EB_Garamond',Georgia,serif] font-normal text-[22px] text-[#0f0f0d]">
          Network
        </Link>
        {!showResults && (
